...
class DepartmentController extends AbstractController
{
    ...
    #[Route('/export', name: 'app_department_export', methods: ['GET'])]
    public function exportEmployees(DepartmentRepository $employeeRepository): Response
    {
        ...
        $csvWriter->insertOne(['ID', 'Department Name']);

        // Insert employee data into the CSV
        foreach ($employees as $employee) {
            $csvWriter->insertOne([$employee->getId(), $employee->getDepartmentName()]);
        }

        // Set response headers
        $response = new Response($csvWriter->toString());
        $response->headers->set('Content-Type', 'text/csv');
        $response->headers->set('Content-Disposition', 'attachment; filename="departments.csv"');
